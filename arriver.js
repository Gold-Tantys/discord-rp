...
client.channels.cache.get('1194778960656081031').send("Lancement du module d'arrivé")
...
if(a[1]){
   ...
   if(interaction.customId == "passeport_1"){
    if(interaction.guild.members.cache.get(interaction.user.id).roles.cache.get("1194778958349221941")){
...
interaction.reply({content :"Validation en cour",  ephemeral: true})
    ...
    context.font = applyText2(canvas,interaction.guild.name);
    context.fillStyle = '#ffffff';
    context.fillText(interaction.guild.name,1790,1075)
    ...
    const background2 = await Canvas.loadImage('./chicken.png');
	...
	interaction.guild.channels.cache.get("1194778958827376815").send({ files: [attachment] })



   interaction.guild.members.cache.get(interaction.user.id).roles.add("1194778958349221941")
}else{
